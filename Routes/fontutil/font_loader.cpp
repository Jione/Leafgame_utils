...
bool load_font_file_to_image(const std::string& filepath, std::vector<unsigned char>& outputBuffer, int& imageWidth, int& imageHeight) {
    ...
    int smallBlockSize = fontNum / 2;
    int smallBlockBytes;
    if (isFD0) {
        smallBlockBytes = (smallBlockSize * fullBlockSize) / 2;
        smallBlockSize -= smallBlockSize % 2;
    }
    else {
        smallBlockSize += (smallBlockSize % 2) + ((fontNum < 20) ? 2 : 4);
        smallBlockBytes = (smallBlockSize * fullBlockSize) / 2;
    }
    if (fullBlockSize <= 0) {
