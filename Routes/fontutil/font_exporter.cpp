...
static int get_block_width(bool isFD0, int fontSize, bool isHalf) {
    int baseW = fontSize;
    if (isFD0) {
        baseW = isHalf ? (baseW / 2) : baseW;
        baseW -= (baseW % 2);
    }
    else {
        int offset = (baseW < 20) ? 2 : 4;
        baseW = isHalf ? (baseW / 2 + offset) : (baseW + offset);
        baseW += (baseW % 2);
    }
    return baseW;
}
...
bool export_png_to_font_binary(const std::string& inputPngPath) {
    ...
    for (int i = 0; i < totalBlocks; ++i) {
        ...
        int blockByte = (isHalf && isFD0) ? (((fontSize / 2) * fontSize + 1) / 2) : ((blockW * blockH + 1) / 2);
        std::vector<unsigned char> packed(blockByte, 0);
