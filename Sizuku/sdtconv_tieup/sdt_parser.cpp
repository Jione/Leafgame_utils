...
namespace {
    ...
    // originalText가 searchText와 일치하는 첫 번째 ParsedString의 id를 반환
    // 못 찾으면 -1 반환
    int findIdByOriginalText(const std::vector<ParsedString>& vec, const std::string& searchText) {
        auto it = std::find_if(vec.begin(), vec.end(), [&](const ParsedString& ps) {
            return ps.originalText == searchText;
            });
        if (it != vec.end())
            return it->id;
        return -1;
    ...
    // 문자열이 "\\n"(백슬래시 + 'n')로 끝나는지 검사
    bool endsWithEscapeNewline(const std::string& s) {
            ...
            case ScriptArgumentType::String2: {
                ...
                if ((argType == ScriptArgumentType::String && strLength >= 0 && strLength < 256) ||
                    ...
                    if (parsedOp.name == "SetMessage" || parsedOp.name == "SetMessage2" ||
                        ...
                        // 테스트용
                        if (parsedOp.name == "SetMessage" || parsedOp.name == "SetMessage2" || parsedOp.name == "SetMessageEx") {
                        ...
                        // 동일한 문장을 하나의 ID로 저장
                        int matchId = -1;
                        matchId = findIdByOriginalText(parsedStrings, newStr.originalText);
                        param.valueType = 1;
                        if ((matchId != -1) && (newStr.originalText.length() > 11)) {
                            param.intValue = matchId;
                        }
                        else {
                            currentStringId++;
