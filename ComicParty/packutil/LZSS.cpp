...
namespace LZSS {
    ...
    {
        if (!entry || !lpOutPath || hSrcFile == INVALID_HANDLE_VALUE) {
            return false;
        }
        ...
        if (entry->Flags == 0) {
            ...
            buffer.resize(entry->DataSize);

            DWORD bytesRead = 0;
            if (entry->DataSize > 0) {
                if (!ReadFile(hSrcFile, buffer.data(),
                    static_cast<DWORD>(entry->DataSize),
                    &bytesRead, NULL) ||
                    bytesRead != entry->DataSize) {
                    return false;
                }
            }

            HANDLE hOut = CreateFileW(lpOutPath,
                GENERIC_WRITE,
                0,
                NULL,
                CREATE_ALWAYS,
        ...
        if (entry->Flags == 1) {
            ...
            // [TotalBlockSize(4)] [UncompressedSize(4)] [LZSS stream...]
            uint32_t blockSize = 0;
            DWORD bytesRead = 0;
            if (!ReadFile(hSrcFile, &blockSize, sizeof(blockSize), &bytesRead, NULL) ||
                bytesRead != sizeof(blockSize)) {
                return false;
            }

            if (blockSize < 8) {
                // Must at least contain uncompressed size + 1 byte of data
                return false;
            }

            uint32_t remaining = blockSize - sizeof(blockSize);
            std::vector<uint8_t> block;
            block.resize(remaining);

            if (remaining > 0) {
                if (!ReadFile(hSrcFile, block.data(), remaining, &bytesRead, NULL) ||
                    bytesRead != remaining) {
                    return false;
                }
            }
            ...
            // First 4 bytes inside block: stored uncompressed size (optional check)
            uint32_t storedUncompressed =
                *reinterpret_cast<const uint32_t*>(block.data());
            uint32_t expectedSize = 0;
            memcpy(&expectedSize, block.data(), sizeof(uint32_t));
            if (expectedSize == 0) {
                expectedSize = storedUncompressed;
            }

            const uint8_t* compData = block.data() + sizeof(uint32_t);
            size_t compSize = remaining - sizeof(uint32_t);

            std::vector<uint8_t> out;
            out.resize(expectedSize);

            if (!DecodeLZSS(compData, compSize, out.data(), expectedSize)) {
                return false;
            }
