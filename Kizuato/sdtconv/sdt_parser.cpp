...
bool SdtParser::parseFile(const std::string& filepath) {
    ...
    while (file.peek() != EOF) {
        ...
        if (!(((static_cast<uint32_t>(ScriptOperator::Start) < opcode) && (opcode < static_cast<uint32_t>(ScriptOperator::OpEnd))) ||
            ...
            std::cerr << "[경고] 부적절한 오퍼레이터 코드. 주소값: " << address << "\n";
            break;
        }
        ...
        for (int paramIndex = 0; paramIndex < 15; ++paramIndex) {
            ...
            case ScriptArgumentType::Compare: {
                ...
                param1.isString = false;
                param1.intValue = ch1;
                param2.isString = false;
                param2.intValue = ch2;

                if (mode == 1) {
                    int32_t num = 0;
                    file.read(reinterpret_cast<char*>(&num), sizeof(num));
                    param3.isString = false;
                    param3.intValue = num;
                }
                else if (mode == 2) {
                    uint8_t length = 0;
            ...
            /* 캐릭터 이름 파싱 시 추후 재새용 가능한 샘플
            if (parsedOp.name == "SetVolumeVoiceChar" && paramIndex == 1) {
                currentCharactorName = find_charactor_name(param.intValue);
            }
            if ((parsedOp.name == "SV" || parsedOp.name == "SVEX") && paramIndex == 2) {
                currentCharactorName = find_charactor_name(param.intValue);
            }
            if ((parsedOp.name == "VV" || parsedOp.name == "VA" || parsedOp.name == "VB" || parsedOp.name == "VC") && paramIndex == 0) {
                currentCharactorName = find_charactor_name(param.intValue);
            } */
        }
